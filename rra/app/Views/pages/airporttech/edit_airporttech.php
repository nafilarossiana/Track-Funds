...
<div class="center">
    ...
    <h4 style="margin-bottom: 5px;">Edit Data Airport Technology Facilities  </h4>
    <p style="margin-top: 5px;">Ubah Data Detail Program dan Anggaran dengan benar </p>
    </div>
      <div class="header-form">
     <button type="button" class="btn custom-button" onclick="redirectToAirportTechPage()">
     <i class="fa fa-arrow-left"></i>&nbsp; &nbsp; Kembali
     </button>
      <br>
      <form action="/pages/airporttech/update/<?= $editairporttech['id_airporttech']; ?>" method="post">
      <?= csrf_field(); ?>
      <input type="hidden" name="id_airporttech" value="<?= $editairporttech['id_airporttech'] ?>">
        <div class="card-body">
          <div class="row">
            <!--kolom 1-->
            <div class="col-md-6">
                 ...
                 <div class="form-group">
                  ...
                  <select id="tahun"  class="selectpicker" data-live-search="true" data-width="100%"  data-none-selected-text="-Pilih Tahun-" name="tahun">
                    ...
                    <?php foreach ($tahun as $key => $value) :?>
                      <option value="<?= $value['tahun']?>" <?=($value['tahun'] == $editairporttech['tahun']) ? 'selected' : null ?>>
                      <?= $value['tahun'] ?>
                      </option>
                    <?php endforeach ?>
                    </select>
                    ...
                    <div class="form-group">
                        <label for="nama_gl" class="d-flex align-items-center">GL-Account</label>
                        <select id="nama_gl" class="selectpicker" data-live-search="true" data-width="100%"  data-none-selected-text="-Pilih Gl-Account-" name="nama_gl">
                            ...
                            <?php foreach ($akununit as $value) : ?>
                                <option value="<?= $value['gl_account'] ?>" <?= $value['gl_account'] == $editairporttech['gl_account'] ? 'selected' : null ?>>
                                <?= $value['gl_account'] ?>
                               </option>
                            <?php endforeach ?>
                        </select>
                    ...
                    <?php foreach ($jenis as $key => $value) :?>
                      <option value="<?= $value['namajenis']?>" <?=($value['namajenis'] == $editairporttech['jenis']) ? 'selected' : null ?>>
                      <?= $value['namajenis'] ?></option>
                    <?php endforeach ?>
                    </select>
                    </div>
                    <div class="form-group">
                    ...
                    <?php foreach ($jeniskontrak as $key => $value) :?>
                      <option value="<?= $value['jenis_kontrak']?>" <?=($value['jenis_kontrak'] == $editairporttech['jeniskontrak']) ? 'selected' : null ?>>
                      <?= $value['jenis_kontrak'] ?></option>
                    <?php endforeach ?>
                    </select>
                    </div>
                    <div class="form-group">
                    ...
                    <?php foreach ($status as $key => $value) :?>
                      <option value="<?= $value['namastatus']?>" <?=($value['namastatus'] == $editairporttech['status']) ? 'selected' : null ?>>
                      <?= $value['namastatus'] ?></option>
                    <?php endforeach ?>
                    </select>
                    </div>
                    <div class="form-group">
                    <label for="nama_program">Nama Program</label>
                    <input type="text" class="form-control" name="nama_program" placeholder="Masukkan Nama Program" value="<?= $editairporttech['detail_program']; ?>">
                    </div>
                    <br>
                </div>
              </div>
            ...
            <div class="col-md-6">
              <div class="form-group">
                ...
                <br>
                <div class="form-group">
                    <label for=nilai_rka ">Nilai RKA</label>
                    <input type="text" class="form-control" name="nilai_rka"  value="<?= $editairporttech['rka']; ?>" readonly>
                    </div>
                <div class="form-group">
                    <label for=nilai_pekerjaan ">Nilai Pekerjaan</label>
                    <input type="text" class="form-control" name="nilai_pekerjaan" placeholder="Masukkan Nilai Pekerjaan" value="<?= $editairporttech['nilai_pekerjaan']; ?>">
                    </div>
                    <div class="form-group">
                    <label for=nilai_program ">Nilai Program</label>
                    <input type="text" class="form-control" name="nilai_program" placeholder="Masukkan Nilai Program" value="<?= $editairporttech['nilai_program']; ?>">
                    </div>
                    <div class="form-group">
                    <label for="geser">Nilai Geseran</label>
                    <input type="text" class="form-control" name="geser" placeholder="Masukkan Nilai Geseran" value="<?= $editairporttech['geser']; ?>">
                    </div>
                    <div class="form-group">
                    <label for="keterangan">Keterangan</label>
                    <input type="text" class="form-control" name="keterangan" placeholder="Masukkan Keterangan" value="<?= $editairporttech['keterangan']; ?>">
                    </div>
                  ...
                  <input type="text" id="januariText" class="form-control" name="januariText" placeholder="Masukkan Nominal Januari" value="<?= $editairporttech['januari']; ?>">
                  </div>
                  <div id="februariInput" class="form-group" style="display: none;">
                  <label for="februariText">Nominal Februari</label>
                  <input type="text" id="februariText" class="form-control" name="februariText" placeholder="Masukkan Nominal Februari" value="<?= $editairporttech['februari']; ?>">
                  </div>
                  <div id="maretInput" class="form-group" style="display: none;">
                  <label for="maretText">Nominal Maret</label>
                  <input type="text" id="maretText" class="form-control" name="maretText" placeholder="Masukkan Nominal Maret" value="<?= $editairporttech['maret']; ?>">
                  </div>
                  <div id="aprilInput" class="form-group" style="display: none;">
                  <label for="aprilText">Nominal April</label>
                  <input type="text" id="aprilText" class="form-control" name="aprilText" placeholder="Masukkan Nominal April"value="<?= $editairporttech['april']; ?>">
                  </div>
                  <div id="meiInput" class="form-group" style="display: none;">
                  <label for="meiText">Nominal Mei</label>
                  <input type="text" id="meiText" class="form-control" name="meiText" placeholder="Masukkan Nominal Mei" value="<?= $editairporttech['mei']; ?>">
                  </div>
                  <div id="juniInput" class="form-group" style="display: none;">
                  <label for="juniText">Nominal Juni</label>
                  <input type="text" id="juniText" class="form-control" name="juniText" placeholder="Masukkan Nominal Juni" value="<?= $editairporttech['juni']; ?>">
                  </div>
                  <div id="juliInput" class="form-group" style="display: none;">
                  <label for="juliText">Nominal Juli</label>
                  <input type="text" id="juliText" class="form-control" name="juliText" placeholder="Masukkan Nominal Juli" value="<?= $editairporttech['juli']; ?>">
                  </div>
                  <div id="agustusInput" class="form-group" style="display: none;">
                  <label for="agustusText">Nominal Agustus</label>
                  <input type="text" id="agustusText" class="form-control" name="agustusText" placeholder="Masukkan Nominal Agustus" value="<?= $editairporttech['agustus']; ?>">
                  </div>
                  <div id="septemberInput" class="form-group" style="display: none;">
                  <label for="septemberText">Nominal September</label>
                  <input type="text" id="septemberText" class="form-control" name="septemberText" placeholder="Masukkan Nominal September" value="<?= $editairporttech['september']; ?>">
                  </div>
                  <div id="oktoberInput" class="form-group" style="display: none;">
                  <label for="oktoberText">Nominal Oktober</label>
                  <input type="text" id="oktoberText" class="form-control" name="oktoberText" placeholder="Masukkan Nominal Oktober" value="<?= $editairporttech['oktober']; ?>">
                  </div>
                  <div id="novemberInput" class="form-group" style="display: none;">
                  <label for="novemberText">Nominal November</label>
                  <input type="text" id="novemberText" class="form-control" name="novemberText" placeholder="Masukkan Nominal November" value="<?= $editairporttech['november']; ?>">
                  </div>
                  <div id="desemberInput" class="form-group" style="display: none;">
                  <label for="desemberText">Nominal Desember</label>
                  <input type="text" id="desemberText" class="form-control" name="desemberText" placeholder="Masukkan Nominal Desember" value="<?= $editairporttech['desember']; ?>">
                  </div>
                  </div>
              <div class="footer">
              <button type="submit" class="btn footer-button"><i class="fa fa-save"></i>&nbsp; &nbsp; Ubah</button>
                </div>
            </div>
          </div>
        </div>
...
<script>
    function redirectToElectricalicalPage() {
        window.location.href = '/electrical/electrical#electrical-month';
    }
</script>
<script>
...
<script>
   var januariValue = <?= json_encode($editairporttech['januari']); ?>;

   var januariIsi = <?= !empty($editairporttech['januari']) ? 'true' : 'false' ?>;

    function showInput() {
        var selectedMonth = document.getElementById("bulan").value;
        if(januariIsi){
          document.getElementById("januariInput").value = januariValue;
        }
