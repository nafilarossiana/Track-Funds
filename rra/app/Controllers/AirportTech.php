...
class AirportTech extends BaseController
{
   
    protected $airporttechModel;
    ...
    public function __construct()
    {
        ...
        $this->airporttechModel = new AirportTechModel();
    ...
    public function airporttech()
    {
        $rkacoba = $this->rkacobaModel->findAll();
        $akununit = $this->akununitModel->findAll();
        $airporttech = $this->airporttechModel->findAll();
        $uniqueYears = $this->airporttechModel->getUniqueYears();
        $uniqueAccount = $this->airporttechModel->getUniqueAccount();
    ...
    if (empty($gl_account)) {
        ...
        $airporttech = $this->airporttechModel->findAll();
            ...
            $airporttech = $this->airporttechModel->getDataByYearAndGLAccount($tahun, $gl_account);
        } elseif ($tahun == 'all' && $gl_account != 'all') {
            // Jika hanya GL account yang dipilih, terapkan filter berdasarkan GL account saja
            $airporttech = $this->airporttechModel->getDataByGLAccount($gl_account);
        } elseif ($tahun != 'all' && $gl_account == 'all') {
            // Jika hanya tahun yang dipilih, terapkan filter berdasarkan tahun saja
            $airporttech = $this->airporttechModel->getDataByYear($tahun);
        }
        
    
    
        foreach($airporttech as &$rc) {
            ...
            $this->airporttechModel->update($rc['id_airporttech'], $updateData);
    
            $rc['realisasi'] = number_format($totalRealisasi, 0, ',', '.');
            $rc['sisa_belum'] = number_format($sisaBelum, 0, ',', '.');
            $rc['sisa_rka'] = number_format($sisaRka, 0, ',', '.');
        }
        $this->airporttechModel->deleteRecordsNotInRkacoba();
        $data = [
            'title' => 'Airport Technology Facilities | RRA Angkasa Pura|Airports',
            'airporttech' => $airporttech, 
        ...
        echo view('pages/airporttech/airporttech', $data);
        echo view('layout/template');
    }
    public function addAirportTech()
{
    $akununit = $this->akununitModel->findAll();
    ...
    $data = [
        'title' => 'Add Airport Technology Facilities | RRA Angkasa Pura|Airports',
        'akununit' => $akununit, 
        'jenis' => $jenis,
    ...
    echo view('pages/airporttech/add_airporttech', $data);
    echo view('layout/template');
}
public function saveAirportTech(){
    ...
    if (!$akunUnitData) {
        ...
        return redirect()->to('/pages/airporttech')->with('error', 'Akun Unit tidak ditemukan untuk GL Account ini');
    }

    $idAkun = $akunUnitData['id_akun'];
    $namaBeban = $akunUnitModel->select('nama_beban')->where('id_akun', $idAkun)->first()['nama_beban'];
    $namaBebanUnit = $namaBeban . " Airport Technology Facilities";

// Mendapatkan data unit dari model UnitModel berdasarkan id_unit
    $unitModel = new UnitModel();
    $defaultUnitData = $unitModel->find(5);

    if (!$defaultUnitData) {
        // Handle jika default unit tidak ditemukan
        return redirect()->to('/pages/airporttech')->with('error', 'Default Unit tidak ditemukan');
    }
    ...
    $this->airporttechModel->insert($data); 
    return redirect()->to('/pages/airporttech')->with('success', 'Data Berhasil Disimpan');
}
public function editAirportTech($id_airporttech){
    $airporttech = $this->airporttechModel->findAll();
    $akununit = $this->akununitModel->findAll();
    $jenis = $this->jenisModel->findAll();
    $jeniskontrak = $this->jeniskontrakModel->findAll();
    ...
    $data['editairporttech'] = $this->airporttechModel->find($id_airporttech);

    $data = [
        'title' => 'Edit Airport Technology Facilities | RRA Angkasa Pura|Airports',
        'airporttech' => $airporttech,
        'akununit' => $akununit, 
        'jenis' => $jenis,
        'jeniskontrak' => $jeniskontrak,  
        ...
        'bulan' => $bulan,
        'tahun' => $tahun,
        'editairporttech' => $data['editairporttech'],

     ];
     echo view('pages/airporttech/edit_airporttech', $data);
     echo view('layout/template');
}
public function updateAirportTech($id_airporttech)
{
    ...
    if (!empty($id_airporttech) && $this->airporttechModel->find($id_airporttech)) {
        // untuk disimpan ke database
        $data = [
            'tahun' => $tahun,
            'id_unit' => $idUnit,
        ...
        $this->airporttechModel->update($id_airporttech, $data);
        return redirect()->to('/pages/airporttech')->with('success', 'Perubahan Data Berhasil Disimpan');
    } else {
        return redirect()->to('/pages/airporttech')->with('error', 'Gagal mengupdate data');
    }
}
public function deleteAirportTech($id_airporttech){
    $this->airporttechModel->delete($id_airporttech);
    return redirect()->to('/pages/airporttech')->with('success', 'Data Berhasil Dihapus');
}

//Tampilan Dashboard Landside 
public function dashAirportTech()
{
    $rekapDataModel = new RekapitulasiModel(); 
    $rekapData = $rekapDataModel->where('id_unit', 5)->findAll();
    $jenisValues = $rekapDataModel->getAllJenisAirportTech();
    $jeniskontrakValues = $rekapDataModel->getAllJenisKontrakAirportTech();
    $statusValues = $rekapDataModel->getAllStatusAirportTech();
    $rrbulanan = $rekapDataModel->getRRAirportTech();
    ...
    $data = [
        'title' => 'Dashboard Aiport Technology | RRA Angkasa Pura|Airports',
        'rekapData' => $rekapData,
...
//dd($data);
    ...
    // Menampilkan view
    echo view('pages/airporttech/dash_airporttech', $data);
    echo view('layout/template');
}

public function dashAirportTechGm()
{
    $rekapDataModel = new RekapitulasiModel(); 
    $rekapData = $rekapDataModel->where('id_unit', 5)->findAll();
    $jenisValues = $rekapDataModel->getAllJenisAirportTech();
...
//dd($data);
    ...
    // Menampilkan view
    echo view('pages/airporttech/dash-airporttech-gm', $data);
    echo view('layout/header-gm');
}

public function exportExcelAiportTech() {
    $landside  = $this->airporttechModel->getExportExcel();
...
$sheet->getStyle('A1:AB1')->getFill()
    ...
    header('Content-Disposition: attachment;filename=AiportTech.xlsx');
