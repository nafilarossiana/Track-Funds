...
class Landside extends BaseController
{
    ...
    public function landside()
    {
        $rkacoba = $this->rkacobaModel->findAll();
        $akununit = $this->akununitModel->findAll();
        $landside = $this->landsideModel->findAll();
        $uniqueYears = $this->landsideModel->getUniqueYears();
        $uniqueAccount = $this->landsideModel->getUniqueAccount();
    ...
    if (empty($gl_account)) {
        ...
        $landside = $this->landsideModel->findAll();
            ...
            $landside = $this->landsideModel->getDataByYearAndGLAccount($tahun, $gl_account);
        } elseif ($tahun == 'all' && $gl_account != 'all') {
            // Jika hanya GL account yang dipilih, terapkan filter berdasarkan GL account saja
            $landside = $this->landsideModel->getDataByGLAccount($gl_account);
        } elseif ($tahun != 'all' && $gl_account == 'all') {
            // Jika hanya tahun yang dipilih, terapkan filter berdasarkan tahun saja
            $landside = $this->landsideModel->getDataByYear($tahun);
        }
        
    
    
        foreach($landside as &$rc) {
        ...
        echo view('pages/landside', $data);
        echo view('layout/template');
    }
    public function addlandside()
{
    $akununit = $this->akununitModel->findAll();
    ...
    $data = [
        'title' => 'Add Landside Facilities | RRA Angkasa Pura|Airports',
        'akununit' => $akununit, 
        'jenis' => $jenis,
    ...
    echo view('pages/landside/addlandside', $data);
    echo view('layout/template');
}
public function saveLandside(){
    ...
    if (!$akunUnitData) {
        ...
        return redirect()->to('/landside/landside')->with('error', 'Akun Unit tidak ditemukan untuk GL Account ini');
    }
...
// Mendapatkan data unit dari model UnitModel berdasarkan id_unit
    ...
    $defaultUnitData = $unitModel->find(2);

    if (!$defaultUnitData) {
        // Handle jika default unit tidak ditemukan
        return redirect()->to('/landside/landside')->with('error', 'Default Unit tidak ditemukan');
    }
    ...
    $this->landsideModel->insert($data); 
    return redirect()->to('/landside/landside')->with('success', 'Data Berhasil Disimpan');
}
public function editLandside($id_landside){
    $landside = $this->landsideModel->findAll();
    $akununit = $this->akununitModel->findAll();
    $jenis = $this->jenisModel->findAll();
    $jeniskontrak = $this->jeniskontrakModel->findAll();
    ...
    $data['editlandside'] = $this->landsideModel->find($id_landside);

    $data = [
        'title' => 'Edit Landside Facilities | RRA Angkasa Pura|Airports',
        'landside' => $landside,
        'akununit' => $akununit, 
        'jenis' => $jenis,
        'jeniskontrak' => $jeniskontrak,  
        ...
        'bulan' => $bulan,
        'tahun' => $tahun,
        'editlandside' => $data['editlandside'],

     ];
     echo view('pages/landside/editlandside', $data);
     echo view('layout/template');
}
public function updateLandside($id_landside)
{
    ...
    if (!empty($id_landside) && $this->landsideModel->find($id_landside)) {
        // untuk disimpan ke database
        $data = [
            'tahun' => $tahun,
            'id_unit' => $idUnit,
        ...
        $this->landsideModel->update($id_landside, $data);
        return redirect()->to('/landside/landside')->with('success', 'Perubahan Data Berhasil Disimpan');
    } else {
        return redirect()->to('/landside/landside')->with('error', 'Gagal mengupdate data');
    }
}
public function deleteLandside($id_landside){
    $this->landsideModel->delete($id_landside);
    return redirect()->to('/landside/landside')->with('success', 'Data Berhasil Dihapus');
}

//Tampilan Dashboard Landside 
public function dashLandside()
{
    $rekapDataModel = new RekapitulasiModel(); 
    $rekapData = $rekapDataModel->where('id_unit', 2)->findAll();
    $jenisValues = $rekapDataModel->getAllJenisLandside();
    $jeniskontrakValues = $rekapDataModel->getAllJenisKontrakLandside();
    $statusValues = $rekapDataModel->getAllStatusLandside();
    $rrbulanan = $rekapDataModel->getRRLandside();
    ...
    $data = [
        'title' => 'Dashboard Landside | RRA Angkasa Pura|Airports',
        'rekapData' => $rekapData,
...
//dd($data);
    ...
    // Menampilkan view
    echo view('pages/landside/dash_landside', $data);
    echo view('layout/template');
}
public function dashLandsideGm()
{
    $rekapDataModel = new RekapitulasiModel(); 
    $rekapData = $rekapDataModel->where('id_unit', 2)->findAll();
    $jenisValues = $rekapDataModel->getAllJenisLandside();
...
//dd($data);
    ...
    // Menampilkan view
    echo view('pages/landside/dash-landside-gm', $data);
    echo view('layout/header-gm');
}

public function exportExcelLandside() {
    $landside  = $this->landsideModel->getExportExcel();
...
$sheet->getStyle('A1:AB1')->getFill()
    ...
    header('Content-Disposition: attachment;filename=Landside.xlsx');
