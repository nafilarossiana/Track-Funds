...
class Electrical extends BaseController
{
    ...
    public function electrical()
    {
        ...
        $electrical = $this->electricalModel->findAll();
        $uniqueYears = $this->electricalModel->getUniqueYears();
        $uniqueAccount = $this->electricalModel->getUniqueAccount();
    ...
    if (empty($gl_account)) {
        ...
        $electrical = $this->electricalModel->findAll();
            ...
            $electrical = $this->electricalModel->getDataByYearAndGLAccount($tahun, $gl_account);
        } elseif ($tahun == 'all' && $gl_account != 'all') {
            // Jika hanya GL account yang dipilih, terapkan filter berdasarkan GL account saja
            $electrical = $this->electricalModel->getDataByGLAccount($gl_account);
        } elseif ($tahun != 'all' && $gl_account == 'all') {
            // Jika hanya tahun yang dipilih, terapkan filter berdasarkan tahun saja
            $electrical = $this->electricalModel->getDataByYear($tahun);
        }
        
    
    
        foreach($electrical as &$rc) {
            ...
            $this->electricalModel->update($rc['id_electrical'], $updateData);
    
            $rc['realisasi'] = number_format($totalRealisasi, 0, ',', '.');
            $rc['sisa_belum'] = number_format($sisaBelum, 0, ',', '.');
            $rc['sisa_rka'] = number_format($sisaRka, 0, ',', '.');
        }
        $this->electricalModel->deleteRecordsNotInRkacoba();
        $data = [
            'title' => 'Electrical Facilities | RRA Angkasa Pura|Airports',
        ...
        echo view('pages/electrical/electrical', $data);
        echo view('layout/template');
    }
    public function addElectrical()
{
    $akununit = $this->akununitModel->findAll();
    ...
    $data = [
        'title' => 'Add Electrical Facilities | RRA Angkasa Pura|Airports',
        'akununit' => $akununit, 
        'jenis' => $jenis,
    ...
    echo view('pages/electrical/add_electrical', $data);
    echo view('layout/template');
}
public function saveElectrical(){
    ...
    if (!$akunUnitData) {
        ...
        return redirect()->to('/pages/electrical')->with('error', 'Akun Unit tidak ditemukan untuk GL Account ini');
    }
...
// Mendapatkan data unit dari model UnitModel berdasarkan id_unit
    ...
    $defaultUnitData = $unitModel->find(4);

    if (!$defaultUnitData) {
        // Handle jika default unit tidak ditemukan
        return redirect()->to('/pages/electrical')->with('error', 'Default Unit tidak ditemukan');
    }
    ...
    $this->electricalModel->insert($data); 
    return redirect()->to('/pages/electrical')->with('success', 'Data Berhasil Disimpan');
}
public function editElectrical($id_electrical){
    $electrical = $this->electricalModel->findAll();
    $akununit = $this->akununitModel->findAll();
    $jenis = $this->jenisModel->findAll();
    $jeniskontrak = $this->jeniskontrakModel->findAll();
    ...
    $data['editelectrical'] = $this->electricalModel->find($id_electrical);

    $data = [
        'title' => 'Edit Electrical Facilities | RRA Angkasa Pura|Airports',
        'electrical' => $electrical,
        'akununit' => $akununit, 
        'jenis' => $jenis,
        'jeniskontrak' => $jeniskontrak,  
        ...
        'bulan' => $bulan,
        'tahun' => $tahun,
        'editelectrical' => $data['editelectrical'],

     ];
     echo view('pages/electrical/edit_electrical', $data);
     echo view('layout/template');
}
public function updateElectrical($id_electrical)
{
    ...
    if (!empty($id_electrical) && $this->electricalModel->find($id_electrical)) {
        // untuk disimpan ke database
        $data = [
            'tahun' => $tahun,
            'id_unit' => $idUnit,
        ...
        $this->electricalModel->update($id_electrical, $data);
        return redirect()->to('/pages/electrical')->with('success', 'Perubahan Data Berhasil Disimpan');
    } else {
        return redirect()->to('/pages/electrical')->with('error', 'Gagal mengupdate data');
    }
}
public function deleteElectrical($id_electrical){
    $this->electricalModel->delete($id_electrical);
    return redirect()->to('/pages/electrical')->with('success', 'Data Berhasil Dihapus');
}

//Tampilan Dashboard Landside 
public function dashElectrical()
{
    $rekapDataModel = new RekapitulasiModel(); 
    $rekapData = $rekapDataModel->where('id_unit', 4)->findAll();
    $jenisValues = $rekapDataModel->getAllJenisElectrical();
    $jeniskontrakValues = $rekapDataModel->getAllJenisKontrakElectrical();
    $statusValues = $rekapDataModel->getAllStatusElectrical();
    $rrbulanan = $rekapDataModel->getRRElectrical();
    ...
    $data = [
        'title' => 'Dashboard Electrical | RRA Angkasa Pura|Airports',
        'rekapData' => $rekapData,
...
//dd($data);
    ...
    // Menampilkan view
    echo view('pages/electrical/dash_electrical', $data);
    echo view('layout/template');
}

public function dashElectricalGm()
{
    $rekapDataModel = new RekapitulasiModel(); 
    $rekapData = $rekapDataModel->where('id_unit', 4)->findAll();
    $jenisValues = $rekapDataModel->getAllJenisElectrical();
...
//dd($data);
    ...
    // Menampilkan view
    echo view('pages/electrical/dash-electrical-gm', $data);
    echo view('layout/header-gm');
}


public function exportExcelElectrical() {
    $landside  = $this->electricalModel->getExportExcel();
...
$sheet->getStyle('A1:AB1')->getFill()
    ...
    header('Content-Disposition: attachment;filename=Electrical.xlsx');
